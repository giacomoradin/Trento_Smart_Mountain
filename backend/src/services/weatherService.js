...
const TOWNS_URL =
  "https://gitlab.com/tinia-euregio/tinia-website/-/raw/main/data/venues/it/2.json";
const POI_URL =
  "https://gitlab.com/tinia-euregio/tinia-website/-/raw/main/data/venues/it/8.json";
...
function parseSlot(key, raw, startDate, intervalMinutes) {
  const { validFrom, validTo } = resolveSlotDates(
    key,
    startDate,
    intervalMinutes,
  );
...
function isCacheValid(forecasts) {
  ...
  if (forecasts.validTo && new Date(forecasts.validTo) < new Date())
    return false;
  return true;
}
...
async function fetchVenues() {
  if (
    _venuesCache &&
    _venuesCacheAt &&
    Date.now() - _venuesCacheAt < VENUES_TTL_MS
  ) {
    return _venuesCache;
  }
  ...
  if (!townsRes.ok)
    throw Object.assign(new Error(`Upstream HTTP ${townsRes.status} (towns)`), {
      statusCode: 502,
    });
  if (!poiRes.ok)
    throw Object.assign(new Error(`Upstream HTTP ${poiRes.status} (poi)`), {
      statusCode: 502,
    });
...
async function fetchForecastFromApi(externalId) {
  ...
  if (!res.ok)
    throw Object.assign(
      new Error(`Upstream HTTP ${res.status} (forecast ${externalId})`),
      { statusCode: 502 },
    );
...
export async function findNearbyVenues(
  lon,
  lat,
  { maxDistance = 50_000, type = null, limit = 10 } = {},
) {
  const filter = {
    location: {
      $near: {
...
export async function seedLocations() {
  ...
  console.log(
    `[weatherService] Seed: ${towns.length} towns, ${pois.length} POI`,
  );
...
export async function refreshLocationForecast(externalId) {
  ...
  if (location.type !== "town") {
    throw Object.assign(new Error(`${externalId} is a POI`), {
      statusCode: 400,
    });
  }
...
export async function getLocationForecast(
  ...
  if (location.type === "town") {
    ...
    if (!town)
      throw new Error(`Reference town not found for POI ${externalId}`);
  }
  ...
  const now = new Date();
  const futureSlots3h = (town.forecasts?.slots3h ?? []).filter(
    (s) => new Date(s.validTo) > now,
  );
  const futureSlots24h = (town.forecasts?.slots24h ?? []).filter(
    (s) => new Date(s.validTo) > now,
  );
