...
describe("Authentication Routes", () => {
  ...
  describe("POST /auth/register/hiker", () => {
    test("should register a new hiker with valid data", async () => {
      ...
      const savedUser = await Hiker.findOne({
        email: "mario.rossi@example.com",
      });
    ...
    test("should fail with duplicate username", async () => {
      ...
      const response = await request(app).post("/auth/register/hiker").send({
        username: "duplicateuser",
      ...
      expect(response.body.message).toMatch(
        /username.*già|email.*username.*registrat|already.*exists/i,
      );
    });
    ...
    test("should fail with duplicate email", async () => {
      ...
      expect(response.status).toBe(409); // Conflict
      expect(response.body.message).toMatch(
        /email.*già|email.*username.*registrat|already.*exists/i,
      );
    });
  ...
  describe("POST /auth/login", () => {
    ...
    test("should fail with incorrect password", async () => {
      ...
      expect(response.body).toHaveProperty("message");
      expect(response.body.message).toMatch(
        /credenziali.*non.*valide|invalid.*credentials/i,
      );
    });
